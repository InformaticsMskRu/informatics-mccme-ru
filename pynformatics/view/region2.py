...
from pynformatics.model import SimpleUser, User, EjudgeContest, Comment, EjudgeProblem, Problem
...
@view_config(route_name='region.submit_test')
def region_submit_test(request):
    try:
        fl = False
        ...
        newpath = '/var/region_result/test_' + time.strftime("%Y%m%d-%H%M%S") + "." + str(random.randint(1,1000))
        ...
        send_mail('ilya.krvn@gmail.com', 'Region results', 
            'URL: <a href="{0}">{0}</a>'.format(url), files)
