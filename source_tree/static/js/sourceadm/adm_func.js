...
function get_children(id) {
    ...
    jQuery.getJSON(get_url() + '/source/get/' + id + '/children', {}, function(res) {
        obj.html('');
        ...
        for (var i = 0; i < res.children.length; ++i) {
            obj.append(jQuery("#child_template").render({
                ...
                problem_id: res.children[i].problem_id,
...
function update_source(id) {
    jQuery.getJSON(get_url() + '/source/get/' + id, {}, function(res) {
        ...
        jQuery("#update_problem").prop("value", res.problem_id);
...
function add_source_submit() {
    jQuery.post(get_url() + '/source/add', {
...
function erase_source(id) {
    resp = confirm("Вы уверены, что хотите удалить ("
        ...
        + "?");
    if (resp) {
        jQuery.post(get_url() + '/source/erase/' + id, {}, function(res){
...
function erase_source_all(id) {
    resp = confirm("Вы уверены, что хотите удалить все поддерево ("
        ...
        + "?");
    if (resp) {
        jQuery.post(get_url() + '/source/erase/' + id + '/all', {}, function(res) {
            parent_id = get_parent(id);
            get_children(parent_id);
            show_alert(res.result, res.content);
        })
...
function make_relation_submit() {
    jQuery.post(get_url() + '/source/update/' + jQuery("#problem_id").prop("value"), {
...
function make_contest(id) {
    ...
    if (resp) {
        ...
        jQuery.post(get_url() + "/contest/add/" + statement_id + "/source/" + id, {}, function(res) {
            show_alert(res.result, res.content);
            get_children(id);
        })
        .fail(send_error);
