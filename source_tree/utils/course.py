...
def course_get_by_id(course_id):
    return db_session.query(CourseTree).filter(
        CourseTree.id == course_id
    ).one()
...
def course_get_by_user(user_id):
    ...
    courses = db_session.query(Course).filter(Course.id.in_(courses_id)).all()
    return courses
...
def course_tree_check_owner(node_id, user_id):
    node_id = int(node_id)
    user_id = int(user_id)
    node = db_session.query(CourseTree).filter(CourseTree.id == node_id).one()
    cap = db_session.query(CourseTreeCap).filter(
        CourseTreeCap.node_id.in_(node.parents()),
        CourseTreeCap.user_id == user_id,
    ).all()
    acc_node = db_session.query(CourseTree).filter(CourseTree.id == cap[0].node_id).one() \
                if cap else None
    return acc_node
...
def course_tree_get_user_nodes(user_id):
    ...
    nodes = set()
    for cap in caps:
        node = db_session.query(CourseTree).filter(
            CourseTree.id == cap.node_id,
        ).one()
        nodes |= set(item for item in node.get_subtree_nodes() if not item.course_id)
    authored_nodes = db_session.query(CourseTree).filter(
        CourseTree.course_id == 0,
        CourseTree.author == user_id,
    ).all()
...
def course_tree_get_root_nodes(user_id):
    ...
    root_nodes = db_session.query(CourseTree).filter(
        CourseTree.id.in_([cap.node_id for cap in caps]),
    )
    return root_nodes

    
def GetNodeUsers(nodeId):
    node = db_session.query(CourseTree).filter(CourseTree.id == nodeId).one()
